...
actor WhisperTranscriber {
    ...
    func transcribe(audioURL: URL, modelURL: URL) async throws -> [TranscriptSegment] {
        let context = try loadContext(for: modelURL)
        ...
        for index in 0..<count {
            ...
            let start = Double(whisper_full_get_segment_t0(context, index)) / 100.0
            let end = Double(whisper_full_get_segment_t1(context, index)) / 100.0
            segments.append(
                TranscriptSegment(start: start, end: end, text: text, speaker: "A")
            )
        }
        return segments
    }
