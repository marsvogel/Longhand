...
//  trailing metadata line — Notes shows the folder there; Kladde shows
...
struct DictationRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            if entry.status != .recording {
                ...
                // left — Kladde's counterpart to Notes' folder line — and
    ...
    private var snippet: String? {
        guard let source = entry.cleanedUp ?? entry.cleanedUpDraft ?? entry.labeledTranscript else {
            return nil
        }
