...
//  WindowTabCommands, the ⌘R command in RecordingCommands, the menu bar item in
//  MenuBarRecordingMenu, and the app-level window lifecycle in AppDelegate.
//

import SwiftUI

@main
struct KladdeApp: App {
    ...
    var body: some Scene {
        ...
        MenuBarExtra {
            MenuBarRecordingMenu(store: store, appDelegate: appDelegate)
        } label: {
            MenuBarRecordingLabel(store: store)
        }
    }
}
