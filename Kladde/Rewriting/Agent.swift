...
//  system prompt, its own reminder line, and its own model id — and
//  LLMProxyClient is only the adapter that calls the CHECK24 LLM Proxy. The two
...
/// One configured agent: the complete system prompt plus the model id
/// LLMProxyClient sends. Pure configuration.
nonisolated struct Agent: Identifiable, Sendable {
    /// Stable identifier for the agent role.
    let id: String
...
nonisolated extension Agent {
    ...
    static let cleanUp = Agent(
        ...
        systemPrompt: """
            ...
            <management-summary>
            The document opens with a management summary, under a heading in the transcript's language, written for a reader who reads nothing else:
            - Three to five sentences, or as few short bullets, on what the dictation was about and what came out of it.
            - Then the next steps — the same ones that appear under <meeting-actions>, each with the owner and the date the transcript names.
            This is the one place that condenses rather than rewrites. Everything it touches still appears in full below it, and nothing appears only here.
            Where the transcript is a short note that settles nothing and leaves nobody anything to do, there is no summary and the document starts with the body.
            </management-summary>
            ...
            <meeting-actions>
            A dictation is often a conversation, and a conversation leaves things behind. After the rewritten body, collect what it left behind under these headings, written in the transcript's language:
            - Decisions — what was settled.
            - Open questions — what was asked and not answered, and what was explicitly left open.
            - Next steps — what somebody said would be done. Name the person the transcript names as the owner, and keep any date or deadline that was said. Where the transcript names nobody, write no owner.
            Each heading appears only when the transcript actually gives it entries; a dictation that gives none of the three ends with the body and nothing after it. Every entry restates something that was said — no invented owners, no invented deadlines, no items inferred from what would plausibly follow. An item collected here stays in the body as well: this is an index of the conversation, not a place to move things to.
            </meeting-actions>

            <speakers>
            The transcript may label turns as **Name:** or **Speaker A:**. Keep those speaker labels in the output. Do not merge different speakers into one voice. Do not invent speakers that are not labeled. When only one unlabeled block is present, treat it as a single speaker and do not invent a name.
            </speakers>
            ...
            <constraints>
            - Add nothing that is not in the transcript: no examples, no justifications, no transitions, no conclusions. Headings and list labels are structure, and apart from the fixed headings in <management-summary> and <meeting-actions> they may only name what the speaker actually said.
            - Do not summarize the body. Every substantive statement is preserved there; only redundancy and the artifacts of speech are cut. The <management-summary> and <meeting-actions> sections are the two places where a statement is restated in short form, and neither ever replaces it in the body.
            ...
            <output>
            Markdown, and the document only. No introduction, no commentary, no code fences, no <transcript> tags. It may open with a `#` title naming the subject; anything else before the management summary is wrong.
        ...
        // CHECK24 LLM Proxy model id (Anthropic Messages API).
